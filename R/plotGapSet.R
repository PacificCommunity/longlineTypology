...
plotGapSet <- function(gap_stat, SE.factor = 1, file = NULL) {
  if (!is.null(file)) grDevices::png(file, 1100, 520, res = 110)
  ...
  graphics::matplot(ks, G, type = "l", lty = 1, col = "#00000030", lwd = 1.5,
          xlab = "k", ylab = "Gap(k)", main = "Gap profiles, 15 replicates")
  graphics::lines(ks, rowMeans(G), col = "firebrick", lwd = 2.5)
  graphics::arrows(ks, rowMeans(G) - rowMeans(S), ks, rowMeans(G) + rowMeans(S),
  ...
  graphics::matplot(ks[-length(ks)], D, type = "l", lty = 1, col = "#00000030", lwd = 1.5,
          xlab = "k", ylab = "Gap(k) - [Gap(k+1) - s(k+1)]",
          main = "Selection criterion (first k above 0 wins)")
  graphics::abline(h = 0, col = "firebrick", lwd = 2, lty = 2)
  sel <- apply(D, 2, function(z) { i <- which(z >= 0); if (length(i)) ks[i[1]] else NA })
  graphics::points(sel, rep(0, length(sel)), pch = 19, col = "firebrick", cex = 1.1)
  graphics::par(op); if (!is.null(file)) grDevices::dev.off()
